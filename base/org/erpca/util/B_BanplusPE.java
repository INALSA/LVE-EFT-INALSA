...
public class B_BanplusPE implements PaymentExport {
	...
	public int exportToFile(MPaySelectionCheck[] checks, File file,
		...
		try {
			...
			for (int i = 0; i < checks.length; i++) {
				...
				// Process Comment
				comment = comment.substring(0, (comment.length() >= 120 ? 120
						: comment.length()));
				...
				String bpTaxID = bp[BPA_A_IDENT_SSN].substring(1,
						(comment.length() >= 10 ? 10 : comment.length()));
				...
				// Credit Detail
				// New Line
				.append(Env.NL).append(personType) // Business Partner Person
													// Type
						.append(separator) // Separator
	 ...
	 * 
	 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> 17/04/2013,
	 *         12:11:16
	 * @param C_BPartner_ID
	 * @return
	 * @return String[]
	...
	private String[] processBPartnerInfo(String[] bp) {
		...
		bpaName = bpaName.substring(0,
				bpaName.length() >= 100 ? 100 : bpaName.length());
