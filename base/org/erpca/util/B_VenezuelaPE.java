...
/**
 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> Export class
 *         for Bank of Venezuela
 */
public class B_VenezuelaPE implements PaymentExport {
	...
	public int exportToFile(MPaySelectionCheck[] checks, File file,
		...
		bankAccountNo = String.format("%1$-" + 20 + "s", bankAccountNo)
				.replace(" ", "0");
		// Batch Document No
		String batchDocNo = String.format("%1$-" + 8 + "s",
				m_PaymentBatch.getDocumentNo()).replace(" ", "0");
		...
		try {
			...
			line.append("HEADER  ") // Constant
					...
					.append(sdf.format(m_PaySelection.getPayDate())) // Payment
																		// Date
					// New Line
					.append(Env.NL);
			//
			...
			for (int i = 0; i < checks.length; i++) {
				...
				/*
				 * // Comment - list of invoice document no StringBuffer comment
				 * = new StringBuffer(); MPaySelectionLine[] psls =
				 * mpp.getPaySelectionLines(false); for (int l = 0; l <
				 * psls.length; l++) { if (l > 0) comment.append(" - ");
				...
				.append("DEBITO  ") // Constant
						...
						.append(docNo) // Document No
						.append(bp[BPA_A_IDENT_SSN]) // Business Partner Account
														// Tax ID
			...
			// Payment Amount
			String totalAmt = String.format("%.2f",
					m_PaySelection.getTotalAmt().abs()).replace(".", ",");
			totalAmt = String.format("%1$" + 18 + "s", totalAmt).replace(" ",
					"0");
	...
	private String[] processBPartnerInfo(String[] bp) {
		...
		bpaTaxID = bpaTaxID.substring(0, bpaTaxID.length() >= 10 ? 10
				: bpaTaxID.length());
		...
		bpaName = bpaName.substring(0,
				bpaName.length() >= 30 ? 30 : bpaName.length());
