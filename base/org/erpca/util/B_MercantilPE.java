...
/**
 * @author <a href="mailto:dixon.22martinez@gmail.com">Dixon Martinez</a> Export
 *         class for Mercantil Bank
 */
public class B_MercantilPE implements PaymentExport {
	 ...
	 */
	public int exportToFile(MPaySelectionCheck[] checks, File file,
			StringBuffer err) {
		...
		//
		MPaySelection m_PaySelection = (MPaySelection) checks[0]
				.getC_PaySelection();
		MPayment m_Payment = (MPayment) checks[0].getC_Payment();
		MPaymentBatch m_PaymentBatch = (MPaymentBatch) m_Payment
				.getC_PaymentBatch();
		MBankAccount m_BankAccount = (MBankAccount) m_PaySelection
				.getC_BankAccount();
		MOrgInfo orgInfo = MOrgInfo.get(m_PaySelection.getCtx(),
				m_PaySelection.getAD_Org_ID(), m_PaySelection.get_TrxName());
		MBank mBank = MBank.get(m_BankAccount.getCtx(),
				m_BankAccount.getC_Bank_ID());
		...
		orgTaxID = orgTaxID.substring(1, (orgTaxID.length() >= 15 ? 15
				: orgTaxID.length()));
		...
		bankIdentification = String.format("%1$-" + 12 + "s",
				bankIdentification);
		...
		// Payment Amount
		String totalAmt = String
				.format("%.2f", m_PaySelection.getTotalAmt().abs())
				.replace(".", "").replace(",", "");
		...
		try {
			...
			line.append("1")
					...
					.append("0000000062")
					// Payment Type
					.append(iden_Type_Org)
					// Ident Type
					.append(orgTaxID)
					// ID Number
					.append(String.format("%0" + 8 + "d", payGenerated))
					// Payments Generated
					.append(totalAmt)
					// Current Date
					.append(sdf.format(m_PaySelection.getPayDate()))
					// Payment Date
					.append(bankAccountNo)
					// Bank Account No
					.append(String.format("%1$" + 7 + "s", arResr).replace(" ",
							"0")) // Reserved
					.append(String.format("%1$" + 8 + "s", arResr).replace(" ",
							"0")) // Reserved Note Serial Number Company
					.append(String.format("%1$" + 4 + "s", arResr).replace(" ",
							"0")) // Reserved Response Code (Data Output)
					.append(String.format("%1$" + 8 + "s", arResr).replace(" ",
							"0")) // Reserved Date process (Data Output)
					.append(String.format("%1$" + 261 + "s", arResr).replace(
							" ", "0")); // Reserved
			...
			for (int i = 0; i < checks.length; i++) {
				...
				docNo = docNo.substring(0,
						(docNo.length() >= 8 ? 8 : docNo.length()));
				docNo = String.format("%1$" + 8 + "s", docNo).replace(" ", "0");
				// Payment Amount
				String amt = String.format("%.2f", m_Payment.getPayAmt().abs())
						.replace(".", "").replace(",", "");
				amt = String.format("%1$" + 17 + "s", amt).replace(" ", "0");

				// Client ID
				/*
				 * String identClientEmpr = bp[BPA_A_IDENT_SSN]; identClientEmpr
				 * = identClientEmpr.substring(0, (batchDocNo.length() >= 16 ?
				 * 16 : identClientEmpr.length())); identClientEmpr =
				 * String.format("%1$-" + 16 + "s", identClientEmpr);
				 */
				String identClientEmpr = orgInfo.getTaxID().replace("-", "")
						.trim();
				identClientEmpr = identClientEmpr.substring(0, (identClientEmpr
						.length() >= 16 ? 16 : identClientEmpr.length()));
				identClientEmpr = String.format("%1$-" + 16 + "s",
						identClientEmpr);
				...
				bPartnerTax = bPartnerTax
						.substring(1, (bPartnerTax.length() >= 15 ? 15
								: bPartnerTax.length()));
				bPartnerTax = String.format("%1$" + 15 + "s", bPartnerTax)
						.replace(" ", "0");
				...
				line.append(Env.NL) // New Line
						...
						.append(tenderType)
						// Tender Type
						.append(String.format("%1$" + 12 + "s", arResr)
								.replace(" ", "0")) // Reserved
						.append(String.format("%1$" + 15 + "s", arResr)) // Reserved
						.append(String.format("%1$" + 15 + "s", arResr)
								.replace(" ", "0")) // Reserved
						...
						.append(String.format("%1$" + 3 + "s", arResr).replace(
								" ", "0")) // Reserved
						...
						.append(String.format("%1$" + 4 + "s", arResr).replace(
								" ", "0")) // Response Code
						.append(String.format("%1$" + 30 + "s", arResr)) // Response
																			// Message
						.append(String.format("%1$" + 80 + "s", arResr)) // Payment
																			// Concept
						.append(String.format("%1$" + 35 + "s", arResr)
								.replace(" ", "0")); // Reserved
	...
	private String[] processBPartnerInfo(String[] bp) {
		...
		bpaName = bpaName.substring(0,
				bpaName.length() >= 60 ? 60 : bpaName.length());
