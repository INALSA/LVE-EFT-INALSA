	...
	public int exportToFile(MPaySelectionCheck[] checks, File file,
		...
		try {
			...
			String totalAmt = String
					...
					.replace(".", "").replace(",", "");
			totalAmt = String.format("%1$" + 13 + "s", totalAmt).replace(" ",
					"0");
			...
			line.append(orgTaxID) // Document No
					...
					.append(String.format("%0" + 4 + "d", payGenerated))// Payments
																		// Generated
			...
			for (int i = 0; i < checks.length; i++) {
				...
				// Credit Detail
				// New Line
				.append(Env.NL).append(bp[BPA_A_NAME]) // Business Partner Name
														// for Account
						.append(amt) // Payment Amount
						.append(comment) // Comment Invoices
						.append(bp[BPA_A_ACCOUNT].substring(1, 4)) // Begin
																	// Account
																	// No
