...
/**
 * Generic Payment Export Sample implementation of Payment Export Interface -
 * brought here from MPaySelectionCheck
 * 
 * @author Jorg Janke
 * 
 ...
 */
public class GenericPaymentVerificationExport implements
		PaymentVerificationExport {
	/** Logger */
	static private CLogger s_log = CLogger
			.getCLogger(GenericPaymentVerificationExport.class);
	...
	public int exportToFile(MPayment[] payments, File file, StringBuffer err) {
		...
		// Must be a file
		if (file.isDirectory()) {
			err.append("No se puede escribir, el archivo seleccionado es un directorio - "
		...
		try {
			...
			line.append(x).append("Value").append(x).append(",").append(x)
					.append("Name").append(x).append(",").append(x)
					.append("Contact").append(x).append(",").append(x)
					.append("Addr1").append(x).append(",").append(x)
					.append("Addr2").append(x).append(",").append(x)
					.append("City").append(x).append(",").append(x)
					.append("State").append(x).append(",").append(x)
					.append("ZIP").append(x).append(",").append(x)
					.append("Country").append(x).append(",").append(x)
					.append("ReferenceNo").append(x).append(",").append(x)
					.append("DocumentNo").append(x).append(",").append(x)
					.append("PayDate").append(x).append(",").append(x)
					.append("Currency").append(x).append(",").append(x)
					.append("PayAmount").append(x).append(",").append(x)
					.append("Comment").append(x).append(Env.NL);
			...
			for (int i = 0; i < payments.length; i++) {
				 ...
				 * for (int l = 0; l < psls.length; l++) { if (l > 0)
				 * comment.append(", ");
				...
				line.append(x)
						...
						.append(",")
						// DocumentNo
						.append(payment.getDateAcct())
						.append(",")
						// PayDate
						.append(x)
						.append(MCurrency.getISO_Code(Env.getCtx(),
								payment.getC_Currency_ID())).append(x)
						.append(",") // Currency
	...
	/**
	 * Get Customer/Vendor Info. Based on BP_ static variables
	 * 
	 * @param C_BPartner_ID
	 *            BPartner
	 * @return info array
