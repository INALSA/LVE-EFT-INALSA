 ...
 * 
 *          Contributors: Carlos Ruiz - GlobalQSS - FR 3132033 - Make payment
 *          export class configurable per bank
 */
public class VPayPrint extends PayPrint implements FormPanel, ActionListener {
	...
	private JButton bProcess = ConfirmPanel.createProcessButton(Msg.getMsg(
			Env.getCtx(), "VPayPrintProcess"));
	...
	private void jbInit() throws Exception {
		...
		//
		centerPanel.add(lPaySelect, new GridBagConstraints(0, 0, 1, 1, 0.0,
				0.0, GridBagConstraints.EAST, GridBagConstraints.NONE,
				new Insets(12, 12, 5, 5), 0, 0));
		centerPanel.add(fPaySelect, new GridBagConstraints(1, 0, 1, 1, 0.0,
		...
		centerPanel.add(fBank, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0,
				...
				new Insets(0, 0, 5, 12), 0, 0));
		centerPanel.add(lPaymentRule, new GridBagConstraints(0, 2, 1, 1, 0.0,
				0.0, GridBagConstraints.EAST, GridBagConstraints.NONE,
				new Insets(0, 12, 5, 5), 0, 0));
		centerPanel.add(fPaymentRule, new GridBagConstraints(1, 2, 1, 1, 0.0,
				0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
				new Insets(0, 0, 5, 12), 0, 0));
		centerPanel.add(lDocumentNo, new GridBagConstraints(0, 3, 1, 1, 0.0,
				0.0, GridBagConstraints.EAST, GridBagConstraints.NONE,
				new Insets(0, 12, 5, 5), 0, 0));
		centerPanel.add(fDocumentNo, new GridBagConstraints(1, 3, 1, 1, 0.0,
				0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
				new Insets(0, 0, 5, 12), 0, 0));
		centerPanel.add(lNoPayments, new GridBagConstraints(2, 3, 1, 1, 0.0,
				0.0, GridBagConstraints.EAST, GridBagConstraints.NONE,
				new Insets(0, 12, 5, 5), 0, 0));
		centerPanel.add(fNoPayments, new GridBagConstraints(3, 3, 1, 1, 0.0,
				0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
				new Insets(0, 0, 5, 12), 0, 0));
	...
	private void loadPaySelectInfo() {
		...
		int C_PaySelection_ID = ((KeyNamePair) fPaySelect.getSelectedItem())
				.getKey();
	...
	private void cmd_export() {
		String PaymentRule = ((ValueNamePair) fPaymentRule.getSelectedItem())
				.getValue();
		...
		if (!getChecks(PaymentRule, trx.getTrxName()))
			return;

		// Yamel Senih 2013-04-16, 19:25 Create Batch before Export as
		// Transaction
		...
		if (m_checks != null && m_checks.length != 0)
			m_batch = new MPaymentBatch(Env.getCtx(), m_checks[0]
					.getC_Payment().getC_PaymentBatch_ID(), trx.getTrxName());

		// Get Current Path
		String pathName = Env.getContext(Env.getCtx(), "#CurrentPath")
				+ File.separator + m_batch.getDocumentNo() + ".txt";
		...
		Env.setContext(Env.getCtx(), "#CurrentPath", fc.getCurrentDirectory()
				.getPath());

		// Create File
		int no = 0;
		StringBuffer err = new StringBuffer("");
		if (m_PaymentExportClass == null
				|| m_PaymentExportClass.trim().length() == 0) {
		...
		try {
			...
			no = -1;
			err.append("No custom PaymentExport class " + m_PaymentExportClass
					+ " - " + e.toString());
			log.log(Level.SEVERE, err.toString(), e);
		} catch (Exception e) {
			no = -1;
			err.append("Error in " + m_PaymentExportClass + " check log, "
					+ e.toString());
			log.log(Level.SEVERE, err.toString(), e);
		}
		if (no >= 0) {
			...
			/*
			 * if (ADialog.ask(m_WindowNo, panel, "VPayPrintSuccess?")) { // int
			 * lastDocumentNo = MPaySelectionCheck.confirmPrint (m_checks,
			 * m_batch); // document No not updated }
			 */
		} else {
	...
	private void cmd_EFT() {
		String PaymentRule = ((ValueNamePair) fPaymentRule.getSelectedItem())
				.getValue();
		log.info(PaymentRule);
		if (!getChecks(PaymentRule, null))
			return;
		// Yamel Senih 2013-04-15, 20:08 Add EFT Support
		// System.err.println(m_batch);
		MPaySelectionCheck.confirmPrint(m_checks, m_batch);
		dispose();
	} // cmd_EFT
	...
	private void cmd_print() {
		...
		// Yamel Senih 2013-05-21, 11:03:10 Hide Ask
		if (!getChecks(PaymentRule, null))
			return;
		...
		if (lastDocumentNo != 0) {
			...
			sb.append("UPDATE C_BankAccountDoc SET CurrentNext=")
					.append(++lastDocumentNo)
					.append(" WHERE C_BankAccount_ID=")
					.append(m_C_BankAccount_ID).append(" AND PaymentRule='")
		...
		for (int i = 0; i < m_checks.length; i++) {
			...
			boolean ok = ReportCtl.startDocumentPrint(ReportEngine.CHECK,
					check.get_ID(), null, Env.getWindowNo(panel), directPrint);
		 ...
		 * m_batch); if (lastDocumentNo != 0) { StringBuffer sb = new
		 * StringBuffer();
		 * sb.append("UPDATE C_BankAccountDoc SET CurrentNext=").
		 * append(++lastDocumentNo)
		...
		boolean confirmRemi = MSysConfig.getBooleanValue(
				"ASK_SEND_REMITTANCE_ON_PAYPRINT", false,
				m_batch.getAD_Client_ID());
		if (confirmRemi
				&& ADialog.ask(m_WindowNo, panel, "VPayPrintPrintRemittance")) {
			for (int i = 0; i < m_checks.length; i++) {
				MPaySelectionCheck check = m_checks[i];
				ReportCtl.startDocumentPrint(ReportEngine.REMITTANCE,
						check.get_ID(), null, Env.getWindowNo(panel),
						directPrint);
			}
		} // remittance
	 ...
	 */
	private boolean getChecks(String PaymentRule, String trxName) {
		// do we have values
		if (fPaySelect.getSelectedIndex() == -1 || m_C_BankAccount_ID == -1
				|| fPaymentRule.getSelectedIndex() == -1
				|| fDocumentNo.getValue() == null) {
			ADialog.error(m_WindowNo, panel, "VPayPrintNoRecords",
					"(" + Msg.translate(Env.getCtx(), "C_PaySelectionLine_ID")
							+ "=0)");
			return false;
		}
		...
		log.config("C_PaySelection_ID=" + C_PaySelection_ID + ", PaymentRule="
				+ PaymentRule + ", DocumentNo=" + startDocumentNo);
		//
		panel.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));

		// get Slecetions
		m_checks = MPaySelectionCheck.get(C_PaySelection_ID, PaymentRule,
				startDocumentNo, trxName);
		// Dixon Martinez
		MPaySelection pay = new MPaySelection(Env.getCtx(), C_PaySelection_ID,
				trxName);
		panel.setCursor(Cursor.getDefaultCursor());
		//
		if (m_checks == null || m_checks.length == 0) {
		...
		m_batch = MPaymentBatch.getForPaySelection(Env.getCtx(),
				C_PaySelection_ID, trxName);
		// Dixon Martinez
		m_batch.setProcessingDate((Timestamp) pay.get_Value("PayDate"));
		m_batch.save();
		return true;
	} // getChecks
